/**
 * Step 5.3 — /logos-status during active intake.
 *
 * Tests:
 * 1. Given intake mode is intake_active and active prompt points to Q1.
 * 2. When handleIntakeCommand({ command: "logos-status" }) runs.
 ...
 * 7. Then status/progress data is returned if available.
 * 8. Then command is not evaluated as an answer.
 * 9. Then state is persisted.
...
describe('handleIntakeCommand — logos-status during active intake', () => {
	it('pauses intake and preserves Q1 before status', async () => {
		...
		expect(result.message.body).toContain('Intake paused before status');
	});

	it('does not advance to Q2 on status', async () => {
		const fs = createFakeFilesystem();
		fs.addStandardProfile();
		const core = createLogosCore({ filesystem: fs });
	...
	it('does not create answer or evaluation records on status', async () => {
		const fs = createFakeFilesystem();
		fs.addStandardProfile();
		const core = createLogosCore({ filesystem: fs });
	...
	it('includes progress in the status message', async () => {
		const fs = createFakeFilesystem();
		fs.addStandardProfile();
		const core = createLogosCore({ filesystem: fs });
		...
		expect(result.message.body).toContain('Intake paused before status');
		expect(result.message.body).toContain('The current prompt was preserved');
	});
});
