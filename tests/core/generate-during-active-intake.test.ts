/**
 * Step 5.3 — /logos-generate during active intake.
 *
 * Tests:
 * 1. Given intake mode is intake_active and active prompt points to Q1.
 * 2. When handleIntakeCommand({ command: "logos-generate" }) runs.
 ...
 * 7. Then no generated paths are written.
 * 8. Then generation/preflight result is blocked/stubbed if full preflight is not implemented.
 * 9. Then command is not evaluated as an answer.
...
describe('handleIntakeCommand — logos-generate during active intake', () => {
	it('pauses intake and preserves Q1 before generation', async () => {
		...
		expect(result.message.body).toContain('Intake paused before generation');
	});

	it('does not advance to Q2 on generate', async () => {
		const fs = createFakeFilesystem();
		fs.addStandardProfile();
		const core = createLogosCore({ filesystem: fs });
	...
	it('does not create answer or evaluation records on generate', async () => {
		const fs = createFakeFilesystem();
		fs.addStandardProfile();
		const core = createLogosCore({ filesystem: fs });
	...
	it('does not write generated documentation paths', async () => {
		const fs = createFakeFilesystem();
		fs.addStandardProfile();
		const core = createLogosCore({ filesystem: fs });
		...
		// No generated docs should be written.
