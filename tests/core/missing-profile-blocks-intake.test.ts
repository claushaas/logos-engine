...
describe('missing profile blocks startIntake (Step 2.4)', () => {
	it('returns blocked when activeProfileId points to missing profile', async () => {
		...
		const initResult = await core.initProject({ projectRoot: '/project' });
		...
		const result = await core.startIntake({ projectRoot: '/project' });
	...
	it('does not persist active question id when profile is missing', async () => {
		const fs = createFakeFilesystem();
		fs.addStandardProfile();
		const core = createLogosCore({ filesystem: fs });
	...
	it('keeps stub blocked behavior when profile is valid', async () => {
		...
		// Full intake is not implemented yet; profile gating must succeed first.
		expect(result.status).toBe('blocked');
		expect(result.blockers.some((b) => b.code === 'profile_not_found')).toBe(
			false,
		);
