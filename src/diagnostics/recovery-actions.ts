...
const CODE_OVERRIDES: Record<string, RecoveryAction[]> = {
	...
	'LOGOS_STATE_MISSING_NODE_DEF_FOR_SYNTHESIS': [
		'open_missing_prerequisite',
	],
 ...
 */
function inferCategoryFromCode(
	code: string,
	...
	if (code.startsWith('LOGOS_PERSISTENCE_') || code.startsWith('LOGOS_SNAPSHOT_')) {
		return 'persistence';
	}
	if (code.startsWith('LOGOS_STALE_') || code.startsWith('LOGOS_MATERIALIZATION_')) {
	...
	if (code.startsWith('LOGOS_APPLY_') || code.startsWith('LOGOS_SUBMIT_') ||
		code.startsWith('LOGOS_SELECT_') || code.startsWith('LOGOS_EDIT_') ||
