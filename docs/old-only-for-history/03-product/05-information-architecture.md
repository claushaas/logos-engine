...
- What exists: workspace state, decisions, assumptions, open questions, risks, canonical documents, Executive JSON, executive exports, derived HTML artifacts, derived agent packs, diagnostics, and configuration.
...
- What is generated: canonical Markdown, Executive JSON, executive export files, HTML artifacts, agent packs, diagnostics, and reports.
...
**Related objects:** Repository Workspace, Canonical Document, Executive Plan, Executive Export, HTML Artifact, Agent Pack, Generation Report.
...
### OBJ-018A: Executive Plan

**User-facing definition:** A portable Executive JSON model generated from the Normative Axis to describe what should happen next.

**Purpose:** Preserve execution structure, readiness, confidence, source normative documents, dependencies, acceptance criteria, risks, decisions, artifacts, and export metadata in a tool-agnostic format.

**Classification:** post-baseline / generated / portable exchange model / derived from normative source.
...
- Overall and area confidence.
- Roadmaps, milestones, workstreams, initiatives, and execution items.
- Decisions, risks, artifacts, dependencies, and acceptance criteria.
...
- Not generated.
- Draft generated.
- Baseline-ready generated.
- Execution-ready generated.
- Stale.
- Blocked.
- Failed.

**Ownership:** Generated by LOGOS from normative documents. User owns whether to use or export it.

**Visibility:** Medium. Visible in generation reports, status, output browser, and future executive review surfaces.

**Permissions relevance:** High when writing files or exporting to external-tool formats.

**Created by:** Executive Axis compilation from normative documents and profile contracts.

**Modified by:** Regeneration after normative documents, profile contracts, or source decisions change.

**Archived or deleted by:** User file management or future cleanup workflow.

**Related objects:** Canonical Document, Document Contract, Generation Report, Executive Export, Risk, Decision, Agent Pack.

**Evidence basis:** Executive Axis Specification and executive generation profile.

**Downstream implications:** Data Model and API Contracts must validate Executive JSON schema and preserve source normative document traceability.

**Open questions:** Whether executive generation is triggered by `/generate` or a dedicated command is unresolved.

### OBJ-018B: Executive Export

**User-facing definition:** A derived snapshot or import file generated from Executive JSON for review, external tool import, or agent execution.

**Purpose:** Transfer execution structure into formats such as Markdown implementation plans, GitHub Issue-compatible Markdown, HTML executive overviews, Linear/Notion payloads when supported, and agent task packs.

**Classification:** post-baseline / derived export / user-facing for advanced use.
...
- Output path.
- Source executive plan path.
- Source normative document references.
- Supported, planned, skipped, blocked, failed, or stale status.
- Generated timestamp.
...
**Ownership:** Generated by LOGOS; live execution ownership moves to the external tool after user import.

**Visibility:** Medium. Visible in generation reports and output lists.

**Permissions relevance:** High because exports may contain sensitive project execution context.

**Created by:** Executive export adapters.

**Modified by:** Regeneration from Executive JSON.

**Archived or deleted by:** User file management.

**Related objects:** Executive Plan, Generation Report, Agent Pack, HTML Artifact, LOGOS Documentation Root.

**Evidence basis:** Executive mappings under `profiles/standard/executive/mappings`.

**Downstream implications:** Integration Architecture must keep exports as file exchange, not live sync. Support must explain that external tools own status after import.

**Open questions:** Which planned adapters are promoted first after Markdown, HTML, GitHub issue files, and agent packs.
...
| REL-022 | Canonical Document | Executive Plan | contributes normative source to | many-to-one | yes | Executive plan derives from the normative baseline, not private chat. | Export requires readiness policy. | Source refs required. |
| REL-023 | Executive Plan | Executive Export | source for | one-to-many | yes | Exports regenerate from Executive JSON and adapter mappings. | Export write consent required. | Derived snapshot only. |
...
| `/generate` | Produce or refresh outputs | Document Contracts and State | Canonical Documents, Executive Plan, Executive Exports, HTML Artifacts, Agent Packs | Review report or diagnostics |
...
- **Output category:** canonical Markdown, Executive JSON, executive export, HTML artifact, agent pack, generation report.
