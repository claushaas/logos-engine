...
| `/init` | Initialize LOGOS workspace, select or confirm active profile, and setup defaults. | Workspace State, Documentation Root, Profile | Repository context available. | Required before creating state or generated root. | Initialized workspace, selected active profile, active root defaulting to `logos/` unless configured. | Preserve existing state; report conflicts. | Local write consent. |
