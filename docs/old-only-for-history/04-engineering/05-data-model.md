...
- contextual suggestions are advisory records or transient view data until the user accepts or edits them through normal answer/proposal capture;
...
| Contextual Suggestion | `ContextualSuggestionRecord` or transient `IntakeResult` item | advisory/proposed | Prior state + current question | Stores or returns suggested answers with source refs, caveats, confidence, and user action status; not confirmed state. |
...
| ContextualSuggestionRecord | IntakeTurnRecord or OpenQuestionRecord | belongs to current question context | many:1 | optional | Suggestion owns advisory status only | Source refs must exist and suggestion status must not imply confirmation. | Rejected/ignored suggestions may be pruned or retained as suppression metadata. |
| ContextualSuggestionRecord | DecisionRecord / AssumptionRecord / OpenQuestionRecord | may create proposal after user acceptance/edit | many:many | optional | Target record owns lifecycle | Accept/edit creates normal answer/proposal state; it cannot create confirmed state directly. | Target records remain if suggestion record is pruned. |
...
| ContextualSuggestionRecord | JSON embedded/session record or transient response | local filesystem or in-memory | Store optional suggested answers and review status for directly related questions. | Contextual Suggestion | Intake | advisory/proposed | proposed, accepted-for-capture, edited-for-capture, rejected, ignored, stale | intake continuation, suggestion suppression, proposal trace | session, source refs, resulting answer/proposal | source basis required; cannot represent confirmed state | suggestion id, question ref, status | sensitive project context | prune/recompute policy review-needed | stale when source refs change | API, Frontend, Test | MVP / validation-required |
...
| Contextual suggestions cannot directly mutate confirmed state. | ContextualSuggestionRecord, DecisionRecord | trust boundary | FR-058, FR-059 | Intake and decision services | Route through answer/proposal capture or reject transition. | immediate | suggestion action tests | API, Frontend, Test |
