...
| API-002 | InitializeWorkspace | internal command | N/A | N/A | Initialize or resume local workspace. | Command Router | Workspace Service | InitializeWorkspace | FR-002, FR-003 | no account auth | user confirmation for initialization/reinit | `InitializeWorkspaceRequest` | `WorkspaceCommandResult` | `workspace_exists`, `unsafe_path`, `state_invalid` | yes | none | none | none | state schema version | WorkspaceInitialized | Must remain idempotent. | MVP |
...
| `GenerateOutputs` | none | profile order then output kind | phase/document/output kind | Standard profile scale | Report every planned output. |
