...
| Markdown, HTML, and agent packs are outputs, not canonical state. | Generated artifacts must not be treated as the source of truth for decisions. | Generation service, status service, downstream docs. | Release-blocking if output parsing becomes canonical state mutation. |
...
| Generated outputs to browser/editor/Git | Markdown, HTML, agent packs. | User controls sharing, but derived outputs can leak context. | Escape HTML, label derived artifacts, no secrets, warn on overwrites/stale outputs. |
