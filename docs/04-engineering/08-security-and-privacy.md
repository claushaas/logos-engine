...
| Contextual suggestions are treated as accepted answers because they fit earlier docs. | Coherent suggestions can create false authority and quietly convert assumptions into decisions. | Require source labels, caveats, optional actions, and normal answer/proposal review before canonical state changes. |
...
| Contextual suggestions are bounded. | Suggestions may use only directly relevant prior state and must remain optional, source-labeled, and non-canonical until reviewed. | Intake/context builder, suggestion service, decision service, UI labels. | Release-blocking if suggestions can bypass review. |
...
| T-013 | Contextual suggestions quietly steer users into prior assumptions. | Overconfident suggestion builder, weak source selection, UI pressure. | Later-phase intake, diagnostics follow-up, proposal review. | User agency, decision quality, validation caveats. | Direct-dependency eligibility, source/caveat labels, accept/edit/reject/ignore actions, normal proposal flow after acceptance. | Suggestion eligibility tests, no-confirmation tests, UI review. | Users may still choose to accept weak suggestions; LOGOS must keep caveats visible. |
...
- source references needed to explain contextual suggestions when a directly related question depends on prior state;
