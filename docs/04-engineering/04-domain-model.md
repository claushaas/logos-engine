...
| Derived Artifact | A non-canonical output generated from canonical source, such as HTML artifact or agent pack. | supporting output | generated artifact | source of truth | Canonical Document, Agent Pack | canonical |
...
| Clarification Aggregate | Intake Session | One active or historical intake flow. | Intake turns, interpreted answer summaries, proposal candidates, low-confidence labels. | Intake may create proposed state only; malformed AI output cannot mutate confirmed state. | Add/interpret turn and create proposals. | add turn, pause, resume, produce proposals. | confirm decisions directly. | IntakeTurnRecorded, DecisionProposed, AssumptionProposed, OpenQuestionCreated |
