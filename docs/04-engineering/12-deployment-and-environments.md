...
| Profile and executive contracts | Bundled `profiles/standard` YAML/schema plus `profiles/standard/executive` mappings/templates/schema as the first supported profile contract set. | Runtime read/package artifact. | Profile loader/schema tests and executive schema/export fixture tests. | Snapshot/contract tests. | Custom profile paths may be sensitive. |
