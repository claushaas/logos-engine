...
Functional Requirements should expand initialization from the target repository directory, LOGOS documentation root configuration with `logos/` as the default, profile selection, AI-led intake, structured state capture, decision review, canonical document rendering, HTML artifact generation, agent pack generation, diagnostics, validation, continuation, and local file management. It should avoid implementation architecture beyond product-level behavior.
